function [M, m, n, code] = zcz_code_L_Zcz(L, Zcz)
%[M, m, n, code] = zcz_code_L_Zcz(L, Zcz)
%
% Returns M and the matrix with the Zero Correlation Zone (ZCZ) codes, from L and Zcz
...
%
%Esp.
% Retorna M y la matriz con los codigos Zero Correlation Zone (ZCZ) a partir de L y Zcz
...
%Calculo de los parametros m y n, a partir de L y Zcz
n = log2(L) - log2(Zcz - 1) - 1;
m = log2(Zcz - 1) - n;
...
% M = 2^(n+1);
% Zcz = 2^(n+m) + 1;
[code, M, ZczRecalculado] = zcz_code(m,n);
